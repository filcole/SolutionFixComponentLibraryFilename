...
Function FixComponentLibraryMetadata {
    ...
    # Use a static placeholder GUID value instead of the random one output each time the solution is exported to avoid dirty diffs
    $placeholderGuid = "deadbeef-dead-beef-dead-beefdeadbeef"
    ...
    Get-ChildItem -Path $canvasFolder -Recurse -File -Filter *.meta.xml | ForEach-Object {
        ...
        $newJson = [regex]::Replace($origJson, $paramBlockPattern, { param($m)
            ...
            $updatedBlock = [regex]::Replace($block, '("workflowName"\s*:\s*\{\s*"value"\s*:\s*")' + $guidRegex + '("\s*\})', '$1' + $placeholderGuid + '$2')
            return $updatedBlock
...
if (!(Test-Path $canvasFolder)) {
    Write-Debug "Canvas app folder does not exist, skipping canvas app fixes."
    return
}

FixComponentLibraryFilenames $canvasFolder
FixComponentLibraryMetadata $canvasFolder